...
@section('content')
    <div class="actions-page">
        <section class="page-hero">
            <div class="container">
                ...
                <a href="{{ route('actions.my-actions') }}" class="btn btn-primary">My Progress</a>
            </div>
        </section>
            ...
            <div class="container">
                <div class="stats-grid-inline">
                    <div class="stat-inline">
                        <span class="stat-value-inline">{{ $actions->count() }}</span>
                        <span class="stat-label-inline">Available Actions</span>
                    </div>
                    <div class="stat-inline">
                        <span class="stat-value-inline">{{ count($completedActionIds) }}</span>
                        <span class="stat-label-inline">Completed</span>
                    </div>
                    <div class="stat-inline">
                        <span
                            class="stat-value-inline">{{ $actions->count() > 0 ? round((count($completedActionIds) / $actions->count()) * 100) : 0 }}%</span>
            ...
            <div class="container">
                <div class="actions-grid">
                    @forelse($actions as $action)
                        <div class="action-card-full {{ in_array($action->id, $completedActionIds) ? 'completed' : '' }}"
                            data-category="{{ $action->category }}">
                            <div class="action-header-full">
                                <span class="action-badge-full badge-{{ $action->category }}">
                            ...
                            <h3 class="action-title-full">{{ $action->title }}</h3>
                            <p class="action-description-full">{{ $action->description }}</p>

                            <div class="action-footer-full">
                                @if (in_array($action->id, $completedActionIds))
                                ...
                                @else
                                    <button class="btn btn-primary"
                                        onclick="openModal({{ $action->id }}, '{{ addslashes($action->title) }}')">
                                        Mark as Done
                                    </button>
                                @endif
                            </div>
    ...
    <div id="actionModal" class="modal" style="display: none;">
        ...
        <div class="modal-content">
            ...
            <form action="{{ route('actions.store') }}" method="POST" class="modal-form">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="completed_at" class="form-input" value="{{ date('Y-m-d') }}"
