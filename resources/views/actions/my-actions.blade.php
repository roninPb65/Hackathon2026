{{-- resources/views/actions/my-actions.blade.php --}}
@extends('layouts.app')

@section('title', 'My Actions')
                ...
                <div class="progress-grid">
                        ...
                        <div class="progress-ring-container">
                            <svg viewBox="0 0 120 120" class="progress-ring">
                                ...
                                <circle cx="60" cy="60" r="54" fill="none" stroke="#6366F1"
                                    stroke-width="8" stroke-dasharray="{{ min($userActions->count() * 5, 100) * 3.39 }} 339"
                                    stroke-linecap="round" transform="rotate(-90 60 60)" />
                            </svg>
                            <div class="progress-text">
                                <span class="progress-number">{{ $userActions->count() }}</span>
                    ...
                    <div class="stats-mini-grid">
                        <div class="stat-mini">
                            ...
                            <div>
                                <div class="stat-mini-value">
                                    {{ $userActions->where('completed_at', '>=', now()->subDays(30))->count() }}</div>
                        ...
                        <div class="stat-mini">
                            ...
                            <div>
                                <div class="stat-mini-value">
                                    {{ $userActions->where('completed_at', '>=', now()->subDays(7))->count() }}</div>
                        ...
                        <div class="stat-mini">
                            ...
                            <div>
                                <div class="stat-mini-value">
                                    {{ $userActions->sum(function ($ua) {return $ua->action->impact_score ?? 0;}) }}</div>
                        ...
                        <div class="timeline-item">
                            ...
                            <div class="timeline-card">
                                <div class="timeline-header">
                                    <span class="timeline-badge badge-{{ $userAction->action->category }}">
                                        {{ ucfirst($userAction->action->category) }}
                                    </span>
                                    <span class="timeline-date">{{ $userAction->completed_at->format('M d, Y') }}</span>
                                </div>

                                <h3 class="timeline-title">{{ $userAction->action->title }}</h3>
                                <p class="timeline-description">{{ $userAction->action->description }}</p>

                                @if ($userAction->reflection)
                                    <div class="reflection-box">
                                        <h4>Your Reflection</h4>
                                        <p>{{ $userAction->reflection }}</p>
                                    </div>
                                @endif
                                ...
                                @if ($userAction->event)
                                    <div class="event-link-box">
                                        ...
                                        <span>Related to: {{ $userAction->event->title }}</span>
                                    </div>
                                @endif
                            </div>
                        </div>
